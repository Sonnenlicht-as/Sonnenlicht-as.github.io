...
作者水平有限，本文内容仅供参考，如有错误。

## 序

在原版Minecraft中，有一些物品（如三叉戟、望远镜）具备这样的特征：在GUI中它显示为2D贴图，在实际持有物品时，显示为3D模型，如果我们也想让自己的物品呈现这样的特性，该如何去实现呢？
在forge加载器下，有两种比较常用的方式来实现这个功能。
一是使用`BlockEntityWithoutLevelRenderer`，二是使用Forge提供的
自定义模型加载器 `SeparateTransformsModelBuilder`。点击此处跳转到利用后者实现功能的教程：[]。
...
现在我们来实现这个类，其构造函数填入Minecraft实例内的两个默认成员即可：
```java
...
```
这里我们采用`Guava`提供的一个 线程安全 + 持久缓存 的函数式接口 `Suppliers.memoize`来创建单例。

## 实现 IClientItemExtensions

我们还需要实现`IClientItemExtensions`接口，以便物品类加载使用。在刚刚创建的类`SpecialItemRenderer`中实现`IClientItemExtensions`：
```java
...
```
我们调用`net.minecraft`包下的工具类`Util`的静态方法`make()`来创建`IClientItemExtensions`的匿名实例，并重写`getCustomRenderer()`方法，返回刚才我们创建的单例。
...
接下来要重写其`initializeClient`方法，该方法的参数接受一个`Consumer<IClientItemExtensions>`类型的对象。
...
##### 创建仅注册物品
为了托管一个在GUI内和实际持有时拥有不同呈现类型的物品的模型，最简单的方法是创建两个物品，一个采用在GUI内时的模型，一个采用实际持有时的模型。
创建仅注册的物品类：
```java
...
`renderStatic`是`ItemRenderer`类中的一个方法，用于渲染物品的烘培模型。`renderStatic`方法的最后一个参数seed填入0，可理解为不使用该参数参与随机运算。
...
- 这里实际上还省略了属性 `render_type`，用于指定烘培模型的渲染类型。
...
```
该模型json可由建模软件如 `BlockBench` 直接生成，大多数情况下，只需对模组ID部分和材质路径部分修改。这一部分的属性的用途，在许多模组教程中都有所提及，这里不再赘述。
...
```
该模型中 `"parent"`属性的值是 `minecraft:item/generated`，代表该模型的模型由minecraft数据包中` generated.json` 提供。诸如`display`等属性已经在 ` generated.json`中进行调整，我们这里只需要指定它的2D材质路径即可。
> 在` generated.json`中，其父模型被指定为`"builtin/generated"`，在原版`ModelBakery`类的`loadBlockModel`方法中，`builtin/generated`是作为普通物品模型加载的`ResourceLocation`（资源定位符）的虚拟路径。该类型的模型 采用`ItemModelGenerator`类的`generateBlockModel`方法、`BlockModel`的`bake`方法 生成体素烘培模型。
{: .prompt-tip }
